...
import 'package:sociall_app_ard/servisler/storageservisi.dart';
...
class FireStoreServisi {
  ...
  Future<int> takipciSayisi(kullaniciId) async {
    QuerySnapshot snapshot = await _firestore
        .collection("takipciler")
        .doc(kullaniciId)
        .collection("kulanicininTakipcileri")
        .get();
  ...
  Future<void> gonderiBegen(Gonderi gonderi, String aktifKullaniciId) async {
    ...
    //Kullanıcı gönderi ilişkisibi begeniler koleksiyonuna ekle
    _firestore
        .collection("begeniler")
        .doc(gonderi.id)
        .collection("doneriBegenileri")
        .doc(aktifKullaniciId)
        .set({});
